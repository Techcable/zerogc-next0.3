    ...
    fn expand_collect_impl(&self) -> Result<TokenStream, Error> {
        ...
        let needs_collect_const = {
            ...
            quote!(const NEEDS_TRACE: bool = {
                ...
                use #copygc_crate::Collect;
                #expr
            };)
        };
        let collected_type = &self.collected_type;
        Ok(quote! {
            unsafe impl #impl_generics #copygc_crate::Collect<#collector_id> for #target_type #where_clause {
                ...
                fn copy_collect<'newgc>(self, context: &mut #copygc_crate::context::CollectContext<'newgc>) -> Self::Collected<'newgc> {
                    #collect_impl
                }
            }
        })
