...
use syn::{Type, GenericArgument, Error, Generics};
use quote::quote;
...
pub fn rewrite_type(target: &Type, target_type_name: &str, rewriter: &mut dyn FnMut(&Type) -> Option<Type>) -> Result<Type, Error> {
...
/// This refers to the copygc crate,
/// equivalent to `$crate` for proc_macros
pub fn copygc_crate() -> TokenStream {
    /*
     * TODO: There is no way to emulate for `$crate` for proc_macros
     *
     * Instead we re-export `extern crate self as copygc` at the root of the copygc crate.
     */
    quote!(copygc::)
}
