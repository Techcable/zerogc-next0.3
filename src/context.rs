...
impl<Id: CollectorId> GarbageCollector<Id> {
    ...
    #[inline]
    unsafe fn alloc_raw<T: RawAllocTarget<Id>>(&self, target: &T) -> (NonNull<T::Header>, NonNull<u8>) {
    ...
    #[cold]
    unsafe fn alloc_raw_fallback<T: RawAllocTarget<Id>>(&self, target: &T) -> (NonNull<T::Header>, NonNull<u8>) {
            ...
            GenerationId::Young => {
                ...
                let (copied_header_ptr, copied_value_ptr) = if array {
                    ...
                    self.garbage_collector
                        ...
                        .map(|(array_header_ptr, value_ptr)| (array_header_ptr.cast::<GcHeader<Id>>(), value_ptr))
                } else {
